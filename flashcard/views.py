...
def novo_flashcard(request):
    ...
    if request.method == "GET":
        ...
        if len(pergunta.strip()) == 0 or len(pergunta.strip()) == 0:
        ...
        messages.add_message(request, constants.SUCCESS, "Flashcard creada con sucesos")
        return redirect('/flashcard/novo_flashcard')
...
def iniciar_desafio(request): 
    ...
    if request.method == "GET":
        ...
        flaschards= Flashcard.objects.filter(user=request.user).filter(dificuldade=dificuldade).filter(categoria_id__in=categorias).order_by('?')
        
        
        if flaschards.count() < int(qtd_perguntas):
            return redirect('/flashcard/iniciar_desafio/')
        
        flaschards = flaschards[: int(qtd_perguntas)]
...
def relatorio(request, id):
    ...
    return render(request, 'relatorio.html', {'desafio': desafio, 'dados': dados, 'name_categoria': name_categoria, 'dados2': dados2 },)
